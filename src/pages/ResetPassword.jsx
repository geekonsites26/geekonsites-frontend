...
export default function ResetPassword() {
  ...
  return <main className="gos-auth-page flex min-h-screen items-center bg-[#edf2f5] px-4 py-6 text-gos-charcoal"><section className="mx-auto w-full max-w-md rounded-lg border border-gos-border bg-white p-5 shadow-[var(--gos-shadow-md)] sm:p-7"><Link to="/customer-login" className="inline-flex items-center gap-2 text-xs font-extrabold text-gos-blue"><ArrowLeft size={15} /> Back to login</Link>{complete ? <div className="py-8 text-center"><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[#eef8f7] text-gos-turquoise"><CheckCircle2 size={24} /></span><h1 className="mt-5 font-['Cormorant_Garamond'] text-4xl font-bold text-gos-blue-deep">Password updated.</h1><p className="mt-3 text-sm font-semibold text-gos-muted">You can now sign in using your new password.</p><Link to="/customer-login" className="mt-7 inline-flex min-h-11 items-center justify-center rounded-lg bg-gos-blue-deep px-6 text-sm font-extrabold text-white">Continue to login</Link></div> : <><div className="mt-7"><p className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-gos-turquoise">Secure account recovery</p><h1 className="mt-2 font-['Cormorant_Garamond'] text-4xl font-bold leading-none text-gos-blue-deep">Create a new password.</h1><p className="mt-3 text-sm font-semibold leading-6 text-gos-muted">Use a new password that you have not used before.</p></div><form onSubmit={submit} className="mt-6 space-y-4">{error && <div role="alert" className="rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm font-bold text-red-700">{error}</div>}<PasswordField label="New password" value={password} onChange={setPassword} show={show} onToggle={() => setShow(!show)} /><p className={`text-[10px] font-bold ${password && !valid ? "text-red-600" : "text-gos-muted"}`}>{valid ? "Strong password" : "Use 8 to 15 characters with uppercase, lowercase, number, and special character."}</p><PasswordField label="Confirm password" value={confirmPassword} onChange={setConfirmPassword} show={show} onToggle={() => setShow(!show)} /><button type="submit" disabled={loading || !token} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-lg bg-gos-blue-deep text-sm font-extrabold text-white disabled:opacity-50">{loading ? <><Loader2 size={17} className="animate-spin" /> Updating...</> : <><Save size={16} /> Update password</>}</button>{!token && <Link to="/forgot-password" className="block text-center text-xs font-extrabold text-gos-turquoise">Request a new reset link</Link>}</form></>}</section></main>
}
