...
export default function ForgotPassword() {
  // A technician who taps "Forgot password?" from the Technician Sign In
  // screen must be returned to Technician Sign In, not Customer Login.
  const { state } = useLocation()
  const isTechnician = state?.role === "TECHNICIAN"
  const loginPath = isTechnician ? "/technician-login" : "/customer-login"
  ...
  return (
    <main className="gos-auth-page flex min-h-screen items-center bg-gos-off-white px-4 py-6 text-gos-charcoal">
      <section className="mx-auto w-full max-w-md rounded-lg border border-gos-border bg-white p-5 shadow-[var(--gos-shadow-md)] sm:p-7">
        <Link to={loginPath} className="inline-flex items-center gap-2 text-xs font-extrabold text-gos-blue"><ArrowLeft size={15} /> Back to login</Link>
        {sent ? (
          <div className="py-8 text-center">
            ...
            <Link to={loginPath} className="mt-7 inline-flex min-h-11 items-center justify-center rounded-lg bg-gos-blue-deep px-6 text-sm font-extrabold text-white">Return to login</Link>
          </div>
        ) : (
          <>
            <div className="mt-7"><p className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-gos-turquoise">Account recovery</p><h1 className="mt-2 font-['Cormorant_Garamond'] text-4xl font-bold leading-none text-gos-blue-deep">Reset your password.</h1><p className="mt-3 text-sm font-semibold leading-6 text-gos-muted">{isTechnician ? "Enter your registered personal email." : "Enter the email used for your GeekOnSites account."}</p></div>
